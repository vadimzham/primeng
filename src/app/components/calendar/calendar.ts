...
export class Calendar implements AfterViewInit,AfterViewChecked,OnInit,OnDestroy,ControlValueAccessor {
    ...
    selectDate(dateMeta) {
        let date
        if (this.utc) date = new Date(Date.UTC(dateMeta.year, dateMeta.month, dateMeta.day));
        else date = new Date(dateMeta.year, dateMeta.month, dateMeta.day);
    ...
    validateMinute(minute): boolean {
        ...
        let valueDateString = value ? value.toDateString() : null;
        if(this.minDate && valueDateString && this.minDate.toDateString() === valueDateString) {
            if(this.getHours(value) == this.getHours(this.minDate)){
        ...
        if(this.maxDate && valueDateString && this.maxDate.toDateString() === valueDateString) {
            if(this.getHours(value) == this.getHours(this.maxDate)){
    ...
    formatTime(date) {
        ...
        if(this.hourFormat == '12') {
            output += this.getHours(date) > 11 ? ' PM' : ' AM';
        }
    ...
    daylightSavingAdjust(date) {
        ...
        this.setHours(date, this.getHours(date) > 12 ? this.getHours(date) + 2 : 0);
